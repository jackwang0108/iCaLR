...
class FullTrainer:
    ...
    def __init__(self, network: nn.Module, dry_run: bool=True):
        ...
        self.optim = optim.SGD(params=self.network.parameters(), lr=0.1, weight_decay=1e-5)
        # self.optim = optim.Adam(params=self.network.parameters(), lr=1e-3, weight_decay=1e-5)
        ...
        # evaluator
        self.train_evaluator = ClassificationEvaluator(num_class=network.num_class)
    ...
    def train(self, n_epoch: int, early_stop: int = 200, plateau: int = 50, show_grid: bool = True):
        ...
        for epoch in (tbar := tqdm.trange(n_epoch)):
            ...
            for step, (x, y) in enumerate(train_loader):
                ...
                # log
                # self.train_evaluator.add_batch_top1(y_pred=y_pred, y=y)
                # self.train_evaluator.add_batch_top5(y_pred=y_pred, y=y)
